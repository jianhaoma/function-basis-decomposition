...
def main():
    ...
    for i in range(len(S)):
        ...
        u = u / torch.norm(u) * 100
        spars.append((torch.trace(F_out[:, :, -1].mm(u))) / 10000)
        spars0.append((torch.trace(F_out[:, :, 0].mm(u))) / 10000)

    top5 = sorted(range(len(spars)), key=lambda i: spars[i], reverse=True)[:5]
    

    for i in range(5):
        u = torch.outer(beta_star[:, top5[i]], U[:, top5[i]])
        u = u / torch.norm(u) * 100
        coe = []
    ...
    data_dict = {'test_acc':test_acc_, 'train_acc': train_acc, 'train_loss':train_loss, 'grad_norm': grad_norm, 'svd_s': S, 'spars': spars, 'spars0': spars0}
